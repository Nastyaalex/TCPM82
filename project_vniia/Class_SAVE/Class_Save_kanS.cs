...
{
    class Class_Save_kanS
    {
        static void CompareRows_kanS(DataTable table_del, DataTable table_in, OleDbDataAdapter adapter, DataTable table_up)
        {
            ...
            foreach (DataRow row_ in table_up.Rows)
            {
                ...
                cmd.CommandText = "UPDATE `КАНы` SET `Номер БД` = ?, `Дата открытия` = ?, `Номер КАНа` = ?, " +
                "`Причина заброкования` = ?, `Состояние` = ?, `Дата закрытия` = ?, `Примечания` = ?, `s_ColLineage` = ?," +
                " `s_Generation` = ?, `s_GUID` = ?, `s_Lineage` = ? WHERE ((`Номер записи` = ?) AND (`Номер БД` = ?)) ";

                cmd.Parameters.AddWithValue("@NumberB", array1[1]);
                cmd.Parameters.AddWithValue("@Data", array1[2]);
                cmd.Parameters.AddWithValue("@NumberKan", array1[3]);
                cmd.Parameters.AddWithValue("@Riason", array1[4]);
                cmd.Parameters.AddWithValue("@Condition", array1[5]);
                cmd.Parameters.AddWithValue("@DataClose", array1[6]);
                cmd.Parameters.AddWithValue("@Prim", array1[7]);
            ...
            foreach (DataRow row_ in table_del.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "DELETE FROM `КАНы` WHERE ((`Номер записи` = ?) AND (`Номер БД` = ?))";
            ...
            foreach (DataRow row_ in table_in.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "INSERT INTO `КАНы` (`Номер БД`, `Дата открытия`, `Номер КАНа`, " +
                "`Причина заброкования`, `Состояние`, `Дата закрытия`, `Примечания`, `s_ColLineage`," +
        ...
        {//сравнение 2-х таблиц
            ...
            CompareRows_kanS(table, table1, adapter, table_up);
