...
{
    class Class_Save_provFey
    {
        static void CompareRows_provFey(DataTable table_del, DataTable table_in, OleDbDataAdapter adapter, DataTable table_up)
        {
            ...
            foreach (DataRow row_ in table_up.Rows)
            {
                ...
                cmd.CommandText = "UPDATE `Проверка ФЭУ` SET `Номер БД` = ?, `Дата` = ?, `Номер ФЭУ` = ?, " +
                "`Напряжение` = ?, `Параметр` = ?, `Значение` = ?, `Примечание` = ?" +
                " WHERE ((`Номер записи` = ?) AND (`Номер БД` = ?)) ";

                cmd.Parameters.AddWithValue("@NumberB", array1[1]);
                cmd.Parameters.AddWithValue("@Data", array1[2]);
                cmd.Parameters.AddWithValue("@NumberFEY", array1[3]);
                cmd.Parameters.AddWithValue("@Napr", array1[4]);
                cmd.Parameters.AddWithValue("@Parametr", array1[5]);
                cmd.Parameters.AddWithValue("@Znach", array1[6]);
            ...
            foreach (DataRow row_ in table_del.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "DELETE FROM `Проверка ФЭУ` WHERE ((`Номер записи` = ?) AND (`Номер БД` = ?))";
            ...
            foreach (DataRow row_ in table_in.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "INSERT INTO `Проверка ФЭУ` (`Номер БД`, `Дата`, `Номер ФЭУ`, " +
                "`Напряжение`, `Параметр`, `Значение`, `Примечание`) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        {//сравнение 2-х таблиц
            ...
            CompareRows_provFey(table, table1, adapter, table_up);
