    ...
    class Class_Save_operMetro
    {
        static void CompareRows_operMetro(DataTable table_del, DataTable table_in, OleDbDataAdapter adapter, DataTable table_up, Dictionary<string, Form1.MyEnd> myEnds)
        {
            ...
            foreach (DataRow row_ in table_up.Rows)
            {
                ...
                cmd.CommandText = "UPDATE `ОперацииМетро` SET `Номер блока` = ?, `Дата` = ?, `Операции` = ? " +
                    "WHERE ((`Номер записи` = ?) AND (`Номер блока` = ?)) ";

                cmd.Parameters.AddWithValue("@NumberB", array1[0]);
                cmd.Parameters.AddWithValue("@Data", array1[2]);
                cmd.Parameters.AddWithValue("@Oper", array1[3]);
            ...
            foreach (DataRow row_ in table_del.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "DELETE FROM `ОперацииМетро` WHERE ((`Номер записи` = ?) AND (`Номер блока` = ?))";

                cmd.Parameters.AddWithValue("@Number", array1[1]);
            ...
            foreach (DataRow row_ in table_in.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "INSERT INTO `ОперацииМетро` (`Номер блока`, `Дата`, `Операции`)" +
                    " VALUES (?, ?, ?)";
