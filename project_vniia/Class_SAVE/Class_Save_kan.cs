...
{
    class Class_Save_kan
    {
        static void CompareRows_Kan(DataTable table_del, DataTable table_in, OleDbDataAdapter adapter, DataTable table_up, Dictionary<string, Form1.MyEnd> myEnds)
        {
            ...
            foreach (DataRow row_ in table_up.Rows)
            {
                ...
                cmd.CommandText = "UPDATE `КАН` SET `Номер КАН` = ?, `Причина Забракования` = ?," +
                    " `Дата Открытия` = ?, `Техническая Сущность` = ?, `Дата Закрытия` = ?, `Примечание` = ?, " +
                    "`s_ColLineage` = ?, `s_Generation` = ?, `s_GUID` = ?, `s_Lineage` = ? " +
                    "WHERE `Номер Записи` = ?";
                
                cmd.Parameters.AddWithValue("@NumberKan", array1[1]);
                cmd.Parameters.AddWithValue("@Riason", array1[2]);
                cmd.Parameters.AddWithValue("@DataO", array1[3]);
                cmd.Parameters.AddWithValue("@TexS", array1[4]);
                cmd.Parameters.AddWithValue("@DataClose", array1[5]);
                cmd.Parameters.AddWithValue("@Prim", array1[6]);
                cmd.Parameters.AddWithValue("@s_C", array1[7]);
                cmd.Parameters.AddWithValue("@s_G", array1[8]);
                cmd.Parameters.AddWithValue("@s_GUID", array1[9]);
                cmd.Parameters.AddWithValue("@s_L", array1[10]);
                cmd.Parameters.AddWithValue("@ID1", array1[0]);
            ...
            foreach (DataRow row_ in table_del.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "DELETE FROM `КАН` WHERE `Номер Записи` = ?";
            ...
            foreach (DataRow row_ in table_in.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "INSERT INTO `КАН` (`Номер КАН`, `Причина Забракования`," +
                    " `Дата Открытия`, `Техническая Сущность`, `Дата Закрытия`, `Примечание`, `s_ColLineage`, " +
        ...
        {//сравнение 2-х таблиц
            ...
            CompareRows_Kan(table, table1, adapter, table_up, myEnds);
