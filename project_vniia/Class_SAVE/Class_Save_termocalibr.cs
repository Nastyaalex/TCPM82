...
{
    class Class_Save_termocalibr
    {
        static void CompareRows_termocalibr(DataTable table_del, DataTable table_in, OleDbDataAdapter adapter, DataTable table_up, Dictionary<string, Form1.MyEnd> myEnds)
        {
            try { 
            ...
            foreach (DataRow row_ in table_up.Rows)
            {
                ...
                cmd.CommandText = "UPDATE `Термокалибровка` SET `Номер БД` = ?, `Дата` = ?, `Температура` = ?, " +
                "`Температура (КОД)` = ?, `Температура (Проц)` = ?, `U (код)` = ?, `U (ШИМ)` = ?, " +
                "`U (измеренное)` = ?, `Код светодиода` = ?, `U (при коде Uном)` = ?, `Примечание` = ?," +
                " `s_ColLineage` = ?, `s_Generation` = ?, `s_GUID` = ?,`s_Lineage` = ?" +
                " WHERE ((`Номер записи` = ?) AND (`Номер БД` = ?)) ";

                cmd.Parameters.AddWithValue("@NumberB", array1[1]);
                cmd.Parameters.AddWithValue("@Data", array1[2]);
                cmd.Parameters.AddWithValue("@Temper", array1[3]);
                cmd.Parameters.AddWithValue("@T_kod", array1[4]);
                cmd.Parameters.AddWithValue("@T_proz", array1[5]);
                cmd.Parameters.AddWithValue("@U_kod", array1[6]);
                cmd.Parameters.AddWithValue("@U_shim", array1[7]);
                cmd.Parameters.AddWithValue("@U_izmer", array1[8]);
                cmd.Parameters.AddWithValue("@Kod_light", array1[9]);
                cmd.Parameters.AddWithValue("@U_Unom", array1[10]);
                cmd.Parameters.AddWithValue("@Prim", array1[11]);
                cmd.Parameters.AddWithValue("@s_C", array1[12]);
                cmd.Parameters.AddWithValue("@s_G", array1[13]);
                cmd.Parameters.AddWithValue("@s_GUID", array1[14]);
                cmd.Parameters.AddWithValue("@s_L", array1[15]);
            ...
            foreach (DataRow row_ in table_del.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "DELETE FROM `Термокалибровка` WHERE ((`Номер записи` = ?) AND (`Номер БД` = ?))";
            ...
            foreach (DataRow row_ in table_in.Rows)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "INSERT INTO `Термокалибровка` (`Номер БД`, `Дата`, `Температура`, " +
                "`Температура (КОД)`, `Температура (Проц)`, `U (код)`, `U (ШИМ)`,`U (измеренное)`," +
                " `Код светодиода`, `U (при коде Uном)`, `Примечание`, `s_ColLineage`," +
        ...
        {//сравнение 2-х таблиц
            ...
            CompareRows_termocalibr(table, table1, adapter, table_up, myEnds);
